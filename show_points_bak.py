...
# python3.7 show_points.py --memefolder=/memes --write_dir=/memes/test -
...
if __name__ == '__main__':
  ...
  parser.add_argument('--weights_path', type=str, default='/workspace/SuperPointPretrainedNetwork/superpoint_v1.pth',
      help='Path to pretrained weights file (default: superpoint_v1.pth).')
  parser.add_argument('--memefolder', type=str, default='/memes',
  ...
  for f in os.listdir(opt.memefolder):
    fp = os.path.join(opt.memefolder, f)
    raw_im = cv2.imread(fp)
    grayim = cv2.imread(fp, 0)
    if grayim is None:
      raise Exception('Error reading image %s' % f)
    grayim = (grayim.astype('float32') / 255.)
    start1 = time.time()
    ...
    for pt in pts.T:
      ...
      cv2.circle(raw_im, pt1, 1, (0, 255, 0), -1, lineType=16)
    out_file = os.path.join(opt.write_dir, f'{f}.png')
    print('Writing image to %s' % out_file)
    cv2.imwrite(out_file, raw_im)